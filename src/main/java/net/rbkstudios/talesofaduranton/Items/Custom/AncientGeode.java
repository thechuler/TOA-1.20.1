...
public class AncientGeode extends Item {


    public AncientGeode(Properties pProperties) {
        super(pProperties);
    }
    ...
    private ItemStack generarLoot(){
        List<Item> DropsTierBajo = Arrays.asList(Items.NETHERRACK,Items.MAGMA_BLOCK,Items.GOLD_NUGGET,Items.SOUL_SAND,Items.BLACKSTONE);

        List<Item> DropsTiermedio = Arrays.asList(Items.GOLD_ORE,Items.NETHER_WART,Items.BLAZE_ROD);
        List<Item> DropsTierAlto = Arrays.asList(Items.RAW_GOLD_BLOCK,Items.NETHERITE_SCRAP);
        List<Item> DropsTierMaximo = Arrays.asList(Items.NETHERITE_INGOT,Items.TOTEM_OF_UNDYING);
